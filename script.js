...
fetch("json/dong_polygon.geojson")
  .then((response) => response.json())
  .then((data) => {
...
fetch("json/dong_nature.geojson")
  .then((response) => response.json())
  .then((data) => {
...
function initialSetLayers() {
  ...
  if (!map.hasImage("dong_icon")) {
    // dong_icon load
    map.loadImage("images/village_icon.png", function (error, image) {
...
function targetByIndex(currentIndex) {
  const targetFeature = features_polygon.find(
    (feature) => feature.properties.Index === currentIndex
  );
...
function loadTargetInfo(target) {
  ...
  document.getElementById("address_des").innerHTML = target.properties.Info;
  ...
  // profile
  const targetProfiles = features_profile.filter(
    (feature) => feature.Address_dong === target.properties.Address_dong
  );
  ...
  const targetNatures = {
    ...
    features: features_nature.filter(
      (feature) =>
        feature.properties.Address_dong === target.properties.Address_dong
    ),
  };
  ...
  const targetDongIcon = {
    type: "FeatureCollection",
    features: features_point.filter(
      (feature) =>
        feature.properties.Address_dong === target.properties.Address_dong
    ),
  };
...
infoXBtn.addEventListener("click", function () {
  ...
  ).getPropertyValue("--popup_width");
  document.getElementById("info-popup").style.right =
    -1.5 * parseInt(popupWidth) + "px";
});
